...
| Date | Change |
|------|--------|
| 2026-04-25 | Audit pass (signup / reviews / images) + reviews aggregate fix + migration collision fix. **Migration chain fix** (2e2d147): previous commit chained `add_qi_card_account_number` behind `a3w4x5y6z7a8` then `b4x5y6z7a8b9` — both already exist on main. Walked the chain with `revision:` + `revision =` parser, confirmed `c5x6y7z8a9b0` (notifications_archive_and_email_pref) as true head, renamed file to `d6y7z8a9b0c1_add_qi_card_account_number.py`. Single-head verified. **Reviews F7 fix** (48661a6): `ReviewService._update_user_rating` now filters `Review.is_public.is_(True)` in its aggregate — previously `users.avg_rating` included hidden reviews but `/reviews/user/{id}` + stats endpoint filtered them out, so the star bar and the review list could disagree after admin moderation. **Audit docs** (new): `docs/launch/signup-audit.md`, `docs/launch/reviews-audit.md`, `docs/launch/images-audit.md`. Signup: no blockers, 4 real gaps (legal acceptance UI-only, is_email_verified hardcoded True, no password-strength meter, no confirm-password). Reviews: **service orders have no review path** (launch blocker — only contract reviews exist; gig-style sellers show 0 reviews forever); 6 P1/P2 gaps incl. no edit window, no seller response, no F6 filter on comment text. Images: security-hardened but no server-side resizing (multi-MB phone photos served 1:1 to 32px avatars), `<img>` not Next `<Image>`, no EXIF strip, no crop UI; 10 findings with a launch-sequencing recommendation. |
