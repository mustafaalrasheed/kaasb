...
21. ~~**Release: Docker build fails on every tag push**~~ — **FIXED** (2026-04-12). Root cause: `release.yml` used raw `docker buildx build --cache-from type=gha` but the GHA cache was written by `docker/build-push-action@v5` (CI) in an incompatible internal format. Error: `checksum of ref: /backend/requirements.txt not found`. Fix: switched release to use `docker/build-push-action@v5` with no cache (releases are tag-triggered and rare — clean reproducible builds are more important than speed).
...
> CI/CD fixes applied 2026-04-11/12: all 3 workflows green. See items 19–21 above.
...
| 20 | CI/CD Pipeline Fixes — All Workflows Green | COMPLETE — mypy.ini, sentry types, npm install, release Docker fix | 2026-04-12 |
