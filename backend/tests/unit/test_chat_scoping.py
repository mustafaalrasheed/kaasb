...
Covers:
- Support inbox scoped to support_assignee_id = me OR NULL (queue)
- claim/release lifecycle
- Rate limit call path on message send
- Conversation model declares the new unique index location + support_assignee_id column
"""
...
class TestSupportInboxScoping:
    """list_support_conversations must filter on support_assignee_id — the
    ...
    async def test_default_scope_filters_to_assigned_or_unassigned(self):
        ...
        # the support_assignee_id filter.
        sql = str(session.execute.await_args_list[0].args[0])
        assert "support_assignee_id" in sql, (
            "support inbox query must filter on support_assignee_id"
        )
    ...
    async def test_only_mine_excludes_unassigned(self):
        ...
        # With only_mine the SQL must use equality on support_assignee_id
...
class TestClaimRelease:
    """claim_support_ticket / release_support_ticket enforce
    ...
    async def test_release_by_non_assignee_raises_forbidden(self):
        ...
        conv.id = uuid.uuid4()
        conv.conversation_type = ConversationType.SUPPORT
        conv.support_assignee_id = uuid.uuid4()  # someone else
        ...
        with pytest.raises(ForbiddenError):
            await svc.release_support_ticket(conv.id, staff)
    ...
    async def test_release_by_assignee_clears_field(self):
        ...
        conv.id = uuid.uuid4()
        conv.conversation_type = ConversationType.SUPPORT
        conv.support_assignee_id = staff.id
        ...
        svc = MessageService(session)
        result = await svc.release_support_ticket(conv.id, staff)
        assert result.support_assignee_id is None
...
class TestConversationModelChanges:
    def test_support_assignee_id_column_exists(self):
        from app.models.message import Conversation

        col = Conversation.__table__.columns["support_assignee_id"]
        assert col.nullable is True
        # Column must have an index (ix_conversations_support_assignee_id).
        idx_names = {idx.name for idx in Conversation.__table__.indexes}
        assert "ix_conversations_support_assignee_id" in idx_names
